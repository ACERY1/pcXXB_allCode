...
$("#ppt-content").load(function(){
    ...
    function putb64(pic,i){
        ...
        var url = "http://upload-z1.qiniu.com/putb64/-1";
            ...
            // if(data.status==0){
                ...
                if(data.courseinfo.knowledge!=undefined){
                    ...
                    for(var i=0;i<knowledge.length;i++){
                        var html='<li data-s="'+knowledge[i].s+'" data-g="'+knowledge[i].g+'" data-id="'+knowledge[i].id+'" data-pid="'+knowledge[i].pid+'" data-pname="'+knowledge[i].pname+'">'+knowledge[i].text+'<img class="icon-remove-circle" src="images/exit.png"></li>';
                        $('.add_point_content ul').append(html);
                        var html2='<option data-id="'+knowledge[i].id+'" data-s="'+knowledge[i].s+'" data-g="'+knowledge[i].g+'" >'+knowledge[i].text+'</option>';
    ...
    switch(stu_subject){
        case '语文':
            subject='1';
            $('.subject-select').find("option[value='1']").prop("selected",true);
            break;
        case '数学':
            subject='2';
            $('.subject-select').find("option[value='2']").prop("selected",true);
            break;
        case '英语':
            subject='3';
            $('.subject-select').find("option[value='3']").prop("selected",true);
            break;
        case '物理':
            subject='4';
            $('.subject-select').find("option[value='4']").prop("selected",true);
            break;
        case '化学':
            subject='5';
            $('.subject-select').find("option[value='5']").prop("selected",true);
            break;
        case '数学':
            subject='6';
            $('.subject-select').find("option[value='6']").prop("selected",true);
            break;
        case '数学':
            subject='7';
            $('.subject-select').find("option[value='7']").prop("selected",true);
            break;
        case '数学':
            subject='8';
            $('.subject-select').find("option[value='8']").prop("selected",true);
            break;
        case '数学':
            subject='9';
            $('.subject-select').find("option[value='9']").prop("selected",true);
            break;
        default:
            subject='2';
            $('.subject-select').find("option[value='2']").prop("selected",true);
    ...
    var init = function(){
        ...
        $.ajax({
            ...
            success:function(data){
                ...
                if(data.pageList[0].imageUrl==""){
                    var pageId = new Array(),imgUrl = new Array();
                    pageId.push(data.pageList[0].id);
                    updateCoursewareImg(pageId,0);
                }
            }
        })
        //查找给该学生上过的课
    ...
    function pointResult(){
        ...
        $("#modal").iziModal('open', function(modal){
            ...
            $.ajax({
                url:'/question/api/getParentPoint',
                type:"get",
                datatype:"json",
                async:false,
                data:data,
    ...
    function getTreeTable(treeList,flag,pName,pId){
        ...
        for(var i=0;i<treeList.length;i++){
            var ifFlag = (treeList[i].isLeaf == 0);
            str = str+'<li><em data-id='+treeList[i].pointId+' data-pName='+pName+' data-pId='+pId+' '+(ifFlag ? '':"class='leaf-node'")+'>'
            +treeList[i].pointName+'</em>'
            +(!ifFlag ? '':'<ul></ul>')+'</li>';
        }
    ...
    $('.point').on('click','.toggler',function(){
        if($(this).siblings('.treemenu').length>0 && $(this).siblings('.treemenu').find('li').length == 0){
            var parentId = $(this).siblings('em').attr('data-id'),
            parentName = $(this).siblings('em').text(),
            $ul = $(this).siblings('.treemenu');
            $.ajax({
                url:'/question/api/getPointByParentId',
                type:'post',
                datatype:'json',
                data:{
                    parentId:parentId
                ...
                success:function(data){
                    ...
                    var array=data.result;
                    if(array.length>0){
                        $ul.html(getTreeTable(array,false,parentName,parentId));
                        $ul.treemenu({delay:300}).openActive();
                        if($ul.find('.leaf-node').length>0){
    ...
    $('#modal').on("click",".leaf-node",function(){
        ...
        $('.point-choose ul').append('<li data-id="'+$(this).attr('data-id')+'"   data-pId="'+$(this).attr('data-pId')+'" data-pName="'+$(this).attr('data-pName')+'">'+$(this).text()+'<img class="icon-remove-circle" src="images/exit.png"></li>');
    ...
    $('.confirm-point').click(function(){
        ...
        $('.point-choose ul li').each(function(){
            ...
            $('.add_point_content ul').append('<li data-id="'+$(this).attr('data-id')+'" data-s="'+subject+'" data-g="'+grade+'" data-pId="'+$(this).attr('data-pId')+'" data-pName="'+$(this).attr('data-pName')+'">'+$(this).text()+'<img class="icon-remove-circle" src="images/exit.png"></li>');
            var html='<option data-id="'+$(this).attr('data-id')+'" data-s="'+subject+'" data-g="'+grade+'">'+$(this).text()+'</option>';
    ...
    function questionResult(pageIndex){
        var data={},diff,style;
        data.thispage=pageIndex;
        data.rowperpage=10;
        data.sort=$('.question-sort img').attr('data-id');
        data.point=$('.point-select').find('option:selected').html();
        // if($('.style-ul li.active').html()!='全部'){
        //     data.fStyleName=$('.style-ul li.active').html();
        // }
        if($('.diff-ul li.active').html()!='全部'){
            data.avgdiff=$('.diff-ul li.active').attr('value');
        }
        // if($('.source-ul li.active').html()!='全部'){
        ...
        data.grade = g!="undefined"?g:grade;
        $.ajax({
            url:"/question/api/questionBankPage",
            type:"get",
            datatype:"json",
            data:data,
            success:function(data){
                var data=JSON.parse(data);
                $('.question-sort p span').html(data.result.countrow);
                $('.totalqpage').html(data.result.countpage);
                ...
                if(data.result.list.length>0){
                    ...
                    var q_array = new Array(),list = data.result.list ,style = data.result.typename;
                    for(var i=0;i<list.length;i++){
                        var html='<li data-id="'+list[i].qid+'"><h3>题目'+(i+1)+':</h3>'+
                        '<div class="stem">'+list[i].question+(list[i].selection!=""?list[i].selection:'')+'</div>'+
                        '<div class="seeQAnswer-content"><span class="question-style" style="display:none;">题型：<span>'+style+'</span></span><span class="question-diff">难度系数：<span>'+list[i].avgDiff+'</span></span><span class="question-frequency">选择次数：<span></span></span><button class="toggle-answer"><i class="icon-angle-down"></i><span>查看解析</span></button></div>'+
                        '<div class="analysis" style="display:none"><h3>试题答案:</h3><p class="question-answer">'+list[i].answer+'</p><h3>试题分析:</h3><div class="question-analysis">'+list[i].ways+'</div><h3>考查知识点:</h3><p class="question-point">'+list[i].secPoint+'</p></div></li>';
                        $('.point-question-content ul').append(html);
                        q_array.push(list[i].qid);
                    }
    ...
    function addQuestion(){
        ...
        if(qtype==2){
            ...
            var question_hd = '<div class="ppt-page-body" data-type="2">';
                    ...
                    question_hd+='<div class="ppt-page-stem" style="overflow:hidden;width:100%;float:left;box-sizing:border-box;font-size:24px;line-height:26px;">';
        ...
        $.ajax({
            ...
            success:function(data){
                ...
                $.ajax({
                    ...
                    datatype:'json',
                    async:false,
                    data:{pageId:data.pageId,htmlContent:iframe.find('.ppt-page-body').eq(totalpage).html(),imageUrl:'http://fs.91xuexibao.com/1501569858359'},
                    success:function(data){
    ...
    function saveKnowledge(){
        ...
        $('.add_point_content ul li').each(function(){
            var knowledge={};
            knowledge.id=$(this).attr('data-id');
            knowledge.pid=$(this).attr('data-pid');
            knowledge.pname=$(this).attr('data-pname');
            knowledge.g=$(this).attr('data-g');
            knowledge.s=$(this).attr('data-s');
    ...
    uploader.on('uploadSuccess', function (file,response,percentage ) {
        ...
        console.log(response);
            ...
            $('.hidImg').on("load",function(){
                ...
                $('.hidImg').remove();
                console.log(iWidth);console.log(iHeight);

                    //html+='<div class="ppt-page-header" style="position:relative;width:100%;border-bottom:1px dashed #dcdcdc;height:5%;"><img src="https://xuexibao1.b0.upaiyun.com/courseware/01cae80659204ae5b388c2fc9d01a8e0.jpg"  />&nbsp;学习宝，用心做辅导</div>';
                var html='<div class="ppt-img" style="width:100%;height:100%;box-sizing:border-box;text-align:center;"><span style="height:100%;display:inline-block;vertical-align:middle;"></span>';
                    html+='<img crossorigin="anonymous" style="display:inline-block;'+(iWidth/iHeight<1.54?'height:100%;margin:0 auto;':'width:100%;vertical-align:middle;')+'" src="https://fs.91xuexibao.com/'+response.key+'"/>';
                    html+='</div>';
                    //html+='<div class="ppt-page-footer" style="position:relative;width:100%;height:5%;font-size:14px;border-top:1px dashed #dcdcdc;line-height:1"><span style="position:absolute;bottom:0;">一对一直播课</span><span style="position:absolute;bottom:0;right:0;"></span></div>';
                ...
                $.ajax({
                    ...
                    data:data,
                    success:function(data){
                        html = '<div class="ppt-page-body" data-type="3">'+html+'</div>';
                        iframe.append(html);
                        iframeWindow.scrollTop(totalpage*768);
                        syncH2C(totalpage)();
                        focusThumbnail(totalpage);
    ...
    function updateCoursewareImg(pageId,activeIndex,async=true){
        setTimeout(function(){
